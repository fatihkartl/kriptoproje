...
4. Giriş ekranındaki fotoğrafı `assets/intro.jpg` yapabilir ya da yine 📷 tuşundan değiştirebilirsin. Arka plan fotoğrafını `.hero` elemanının stilindeki `--hero-image` ile güncellersin.
5. Arka plan müziği için `assets/music.mp3` dosyasını ekle (play tuşu müziği ve geçiş animasyonunu tetikler).
...
- `assets/`: Fotoğraflar ve `music.mp3` için klasör. Varsayılan örnek görseller `sample01.jpg` … `sample18.jpg` olarak referanslandı.

### İpuçları
- Yeni kart eklemek istersen `timeline-feed` içindeki `<li class="timeline-item ...">` bloklarını çoğaltıp `data-edit-key` / `data-upload-key` değerlerini benzersiz yap.
